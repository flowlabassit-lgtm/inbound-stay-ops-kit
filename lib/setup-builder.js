...
function normalizeWifiSecurity(value) {
  const security = cleanText(value);
  return ["WPA", "WEP", "nopass"].includes(security) ? security : "WPA";
}
...
export function buildConfigDraft(input = {}) {
  ...
  const wifiSsid = cleanText(input.wifiSsid);
  const wifiEnabled = isChecked(input.wifiQrEnabled) && Boolean(wifiSsid);
  ...
  return {
    ...
    wifiQr: {
      ...
      hostApproved: false,
      ssid: wifiSsid,
      password: wifiEnabled ? cleanText(input.wifiPassword) : "",
      security: normalizeWifiSecurity(input.wifiSecurity),
      hidden: isChecked(input.wifiHidden),
      showPasswordText: false,
      note: "Host must approve this before publishing. Do not commit real Wi-Fi passwords to a public repository."
    },
    agent: {
      enabled: false,
      endpoint: "",
...
export function buildSetupPrompt(input = {}) {
  ...
  const wifiRequested = isChecked(input.wifiQrEnabled);
  const wifiPasswordProvided = Boolean(cleanText(input.wifiPassword));
  ...
  return [
    ...
    "- If Wi-Fi QR is requested, keep wifiQr.hostApproved false until the host approves it, keep showPasswordText false by default, and never publish real Wi-Fi passwords to public repositories.",
    ...
    `Wi-Fi QR requested: ${wifiRequested ? "yes" : "no"}`,
    `Wi-Fi SSID: ${cleanText(input.wifiSsid)}`,
    `Wi-Fi security: ${normalizeWifiSecurity(input.wifiSecurity)}`,
    `Wi-Fi hidden network: ${isChecked(input.wifiHidden) ? "yes" : "no"}`,
    `Wi-Fi password provided: ${wifiPasswordProvided ? "yes" : "no"}`,
    "Wi-Fi password value is intentionally omitted from this prompt. Keep it local/private in config.json only if the host approves the QR workflow.",
    "",
